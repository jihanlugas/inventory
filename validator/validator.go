...
func init() {
	...
	regHiragana = regexp.MustCompile(`^[ぁ-ん.。\s|-]+$`)
	regKatakana = regexp.MustCompile(`^[ァ-ン.。\s|-]+$`)
	regKana = regexp.MustCompile(`^[ぁ-んァ-ン.。\s|-]+$`)
	regKanji = regexp.MustCompile(`^[一-龯.。\s|-]+$`)
}
...
func notExistsOnDbTable(fl validator.FieldLevel) bool {
	...
	case "word":
		...
		var cnt int
		row := conn.QueryRow(ctx, `SELECT count(*) FROM public.msword WHERE word=$1`, word)
		if err = row.Scan(&cnt); err != nil {
			return false
		}
		return cnt == 0
